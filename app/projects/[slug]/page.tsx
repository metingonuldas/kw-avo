...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  if (!data) {
    return {
      ...
      description:
        "KWAVO danışmanlığıyla İzmir ve yurt içi/yurt dışı gayrimenkul projelerini keşfedin.",
  ...
  const typeText =
    p.type === "villa"
      ? "Villa projesi"
      : p.type === "visa"
      ? "Golden Visa ve yatırım odaklı proje"
      : "Konut projesi";

  const title = `${p.title}`;
  const desc = `${location ? location + " – " : ""}${
    p.developer ? p.developer + " geliştiricili " : ""
  }${typeText}. Detaylı bilgi ve randevu için KWAVO ekibiyle iletişime geçin.`;

  const ogImage = p.hero
    ? p.hero
    : `/og?title=${encodeURIComponent(p.title)}`;
    ...
    <>
      ...
      <Script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            description: `${p.title} – ${
              p.city || ""
            } ${p.country || ""} lokasyonunda ${
              p.type === "villa"
                ? "villa projesi"
                : p.type === "visa"
                ? "yatırım / Golden Visa projesi"
                : "konut projesi"
            }.`,
            address: {
              "@type": "PostalAddress",
            ...
            offers: p.price_from
              ? {
                  "@type": "Offer",
                  price: p.price_from.replace(/[^\d]/g, ""),
                  priceCurrency: p.price_from.includes("€")
                    ? "EUR"
                    : p.price_from.includes("$")
                    ? "USD"
                    : "TRY",
      ...
      <main className="mx-auto max-w-6xl px-4 sm:px-6 py-10">
        ...
        <section className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          {p.developer && (
            <SummaryItem label="Geliştirici" value={p.developer} />
          )}
          {p.price_from && (
            <SummaryItem label="Başlangıç" value={p.price_from} />
          )}
        </section>
