...
export default function CareerAssistant({
  ...
  return (
    <section className="mt-6 overflow-hidden rounded-[2rem] border border-black/10 bg-white shadow-sm">
      <div className="flex flex-col gap-4 border-b border-black/8 bg-[#1e1b1c] px-5 py-5 text-white sm:flex-row sm:items-center sm:justify-between sm:px-7">
        <div className="flex items-center gap-3">
          <div className="relative flex h-12 w-12 items-center justify-center rounded-2xl bg-[#ba0c2f]"><Bot size={25} /><span className="absolute -bottom-1 -right-1 h-3.5 w-3.5 rounded-full border-2 border-[#1e1b1c] bg-emerald-400" /></div>
          <div><div className="flex items-center gap-2"><h2 className="font-black">AVO Kariyer Asistanı</h2><span className="rounded-full bg-white/10 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-white/60">Yapay zekâ</span></div><p className="mt-1 text-xs text-white/55">Profilinizi ve danışmanlık modelini birlikte konuşun</p></div>
        </div>
      ...
      <div ref={listRef} className="h-[430px] space-y-5 overflow-y-auto bg-[#f7f5f2] px-4 py-6 sm:px-7" aria-live="polite">
        {messages.map((message, index) => (
          <div key={`${message.role}-${index}`} className={`flex gap-3 ${message.role === "user" ? "justify-end" : "justify-start"}`}>
            ...
            <div className={`max-w-[82%] whitespace-pre-wrap rounded-2xl px-4 py-3 text-sm leading-6 shadow-sm ${message.role === "user" ? "rounded-br-md bg-[#ba0c2f] text-white" : "rounded-bl-md border border-black/8 bg-white text-neutral-700"}`}>
            ...
            {message.role === "user" && <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-[#ba0c2f]/10 text-[#ba0c2f]"><UserRound size={17} /></div>}
          </div>
        ))}
        {messages.length === 1 && (
          <div className="pt-2">
            ...
            <div className="flex flex-wrap gap-2">{STARTERS.map((starter) => <button key={starter} onClick={() => void sendMessage(starter)} className="rounded-full border border-black/10 bg-white px-3 py-2 text-left text-xs font-semibold text-neutral-600 transition hover:border-[#ba0c2f]/40 hover:text-[#ba0c2f]">{starter}</button>)}</div>
          </div>
        )}
      </div>

      <form onSubmit={submit} className="border-t border-black/8 bg-white p-4 sm:p-5">
        <div className="flex items-end gap-3 rounded-2xl border border-black/15 bg-white p-2 pl-4 focus-within:border-[#ba0c2f] focus-within:ring-2 focus-within:ring-[#ba0c2f]/10">
          <textarea value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void sendMessage(input); } }} rows={2} maxLength={1600} disabled={loading} placeholder="AVO'ya sorunuzu yazın…" className="max-h-32 min-h-12 flex-1 resize-none bg-transparent py-2 text-sm outline-none disabled:opacity-60" aria-label="AVO'ya mesajınız" />
        ...
        <div className="mt-2 flex items-center justify-between gap-3 text-[11px] text-neutral-400"><span>Enter: gönder · Shift + Enter: yeni satır</span><span className="inline-flex items-center gap-1"><MessageCircle size={12} /> AVO hata yapabilir; önemli bilgileri uzmanla teyit edin.</span></div>
