...
function ResultBars({ percentages }: { percentages: Record<StyleKey, number> }) {
  return (
    <div className="space-y-3">
      {STYLE_ORDER.map((key) => (
        <div key={key} className="grid grid-cols-[88px_1fr_44px] items-center gap-3 text-sm">
          <span className="font-semibold">{PROFILES[key].name}</span>
...
export default function CareerCompass() {
  ...
  function showIntake() {
    ...
    setPhase("intake");
    window.setTimeout(() => document.getElementById("aday-formu")?.focus(), 50);
  }
  ...
  function beginQuiz() {
    ...
    setCurrent(0);
    setAnswers([]);
    window.setTimeout(() => document.getElementById("pusula-kart")?.focus(), 50);
  }
  ...
  function choose(style: StyleKey) {
    ...
    if (current === QUESTIONS.length - 1) {
      setPhase("result");
      window.setTimeout(() => document.getElementById("sonuc")?.focus(), 50);
    } else {
  ...
  if (phase === "intro") {
    return (
      <main className="min-h-[calc(100vh-64px)] overflow-hidden bg-[#f5f1eb] text-[#181515]">
        <section className="relative mx-auto grid min-h-[760px] max-w-7xl items-center gap-12 px-5 py-14 sm:px-8 lg:grid-cols-[1.08fr_0.92fr] lg:px-12">
          ...
          <div className="relative">
            ...
            <h1 className="mt-7 max-w-3xl text-5xl font-black leading-[0.98] tracking-[-0.05em] sm:text-6xl lg:text-7xl">Gayrimenkulde nasıl bir iz bırakırsınız?</h1>
            <p className="mt-6 max-w-xl text-lg leading-8 text-neutral-600">12 kısa senaryoda doğal çalışma stilinizi keşfedin. Sonuçta size özel güçlü yönler, gelişim ipuçları ve danışmanlık yol haritası alın.</p>
            <button onClick={showIntake} className="mt-8 inline-flex items-center gap-3 rounded-2xl bg-[#ba0c2f] px-6 py-4 font-bold text-white shadow-[0_16px_45px_rgba(186,12,47,0.28)] transition hover:-translate-y-0.5 hover:bg-[#a00a29]">
          ...
          <div className="relative rounded-[2rem] border border-black/10 bg-[#1e1b1c] p-4 shadow-2xl sm:p-6">
            <div className="rounded-[1.5rem] bg-white p-6 sm:p-8">
              ...
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                {STYLE_ORDER.map((key) => (
                  <article key={key} className="rounded-2xl border border-black/10 p-4">
                    <div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl text-sm font-black text-white" style={{ backgroundColor: PROFILES[key].color }}>{key}</span><h3 className="font-bold">{PROFILES[key].name}</h3></div>
                    <p className="mt-3 text-sm leading-5 text-neutral-500">{PROFILES[key].tagline}</p>
                  </article>
                ))}
              </div>
            </div>
  ...
  if (phase === "intake") {
    const fieldClass = "mt-1.5 w-full rounded-xl border border-black/15 bg-white px-3 py-3 font-normal outline-none transition focus:border-[#ba0c2f] focus:ring-2 focus:ring-[#ba0c2f]/10";
    return (
      <main className="min-h-[calc(100vh-64px)] bg-[#f5f1eb] px-4 py-10 sm:px-6 lg:py-14">
        <section id="aday-formu" tabIndex={-1} className="mx-auto max-w-4xl outline-none">
          <button onClick={() => setPhase("intro")} className="mb-5 inline-flex items-center gap-2 text-sm font-semibold text-neutral-500 hover:text-black"><ArrowLeft size={17} /> Geri dön</button>
          <div className="overflow-hidden rounded-[2rem] border border-black/10 bg-white shadow-xl">
            <div className="border-b border-black/8 bg-[#1e1b1c] px-6 py-7 text-white sm:px-9">
              <p className="text-xs font-bold uppercase tracking-[0.18em] text-white/55">Adım 1 / 2</p>
              <h1 className="mt-2 text-3xl font-black">Sizi biraz tanıyalım</h1>
            ...
            <form onSubmit={submitIntake} className="grid gap-5 p-6 sm:grid-cols-2 sm:p-9">
              ...
              <fieldset className="sm:col-span-2">
                ...
                <div className="mt-3 flex gap-6 text-sm"><label className="flex items-center gap-2"><input name="entrepreneurship" type="radio" value="Hayır" required /> Hayır</label><label className="flex items-center gap-2"><input name="entrepreneurship" type="radio" value="Evet" required /> Evet</label></div>
              </fieldset>
              <input name="website" tabIndex={-1} autoComplete="off" className="absolute -left-[9999px] h-px w-px" aria-hidden="true" />
              <label className="flex items-start gap-2 rounded-xl bg-neutral-50 p-4 text-xs leading-5 text-neutral-600 sm:col-span-2"><input name="consent_terms" type="checkbox" required className="mt-1" /><span>Bilgilerimin başvuru sürecinin yürütülmesi ve benimle iletişim kurulması amacıyla işlenmesini kabul ediyorum. <a href="/privacy" target="_blank" className="font-semibold underline">Gizlilik Politikası</a></span></label>
              <div className="flex flex-col-reverse gap-3 border-t border-black/8 pt-5 sm:col-span-2 sm:flex-row sm:items-center sm:justify-between">
                ...
                <button disabled={intakeSubmitting} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#ba0c2f] px-6 py-3.5 font-bold text-white transition hover:bg-[#a00a29] disabled:opacity-60">{intakeSubmitting ? "Kaydediliyor…" : "Kaydet ve teste geç"}<ArrowRight size={18} /></button>
              </div>
  ...
  if (phase === "quiz") {
    ...
    return (
      <main className="flex min-h-[calc(100vh-64px)] items-center bg-[#f5f1eb] px-4 py-10 sm:px-6">
        <section id="pusula-kart" tabIndex={-1} className="mx-auto w-full max-w-4xl outline-none">
          <div className="mb-5 flex items-center justify-between text-sm font-semibold text-neutral-500"><span>{question.context}</span><span className="tabular-nums">{current + 1} / {QUESTIONS.length}</span></div>
          ...
          <div className="mt-6 rounded-[2rem] border border-black/10 bg-white p-5 shadow-xl sm:p-9">
            <h1 className="max-w-3xl text-2xl font-black leading-tight sm:text-3xl">{question.prompt}</h1>
            <p className="mt-3 text-sm text-neutral-500">Size en yakın olan seçeneği düşünmeden, doğal refleksinize göre işaretleyin.</p>
            <div className="mt-7 grid gap-3 sm:grid-cols-2">
              {question.answers.map((answer) => (
                <button key={answer.style} onClick={() => choose(answer.style)} className={`group flex min-h-24 items-center justify-between gap-4 rounded-2xl border p-5 text-left text-[15px] font-medium leading-6 transition hover:-translate-y-0.5 hover:border-[#ba0c2f]/50 hover:bg-[#fff8f9] ${answers[current] === answer.style ? "border-[#ba0c2f] bg-[#fff8f9]" : "border-black/10"}`}>
            ...
            <div className="mt-7 flex items-center justify-between border-t border-black/8 pt-5">
              <button disabled={current === 0} onClick={() => setCurrent((value) => Math.max(0, value - 1))} className="inline-flex items-center gap-2 text-sm font-semibold text-neutral-500 disabled:invisible"><ArrowLeft size={17} /> Geri</button>
  ...
  return (
    <main id="sonuc" tabIndex={-1} className="bg-[#f5f1eb] px-4 py-10 outline-none sm:px-6 lg:py-16">
      <section className="mx-auto max-w-6xl">
        <div className="grid overflow-hidden rounded-[2rem] border border-black/10 bg-white shadow-xl lg:grid-cols-[0.82fr_1.18fr]">
          <div className="relative flex min-h-[390px] flex-col justify-between p-7 text-white sm:p-10" style={{ backgroundColor: primary.color }}>
            ...
            <div className="relative">
              ...
              <div className="mt-8 flex h-24 w-24 items-center justify-center rounded-3xl bg-white text-5xl font-black shadow-lg" style={{ color: primary.color }}>{result.primary}</div>
            </div>
            <div className="relative mt-12">
              <h1 className="text-5xl font-black tracking-[-0.04em]">{primary.name}</h1>
          ...
          <div className="p-6 sm:p-10">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div><p className="text-xs font-bold uppercase tracking-[0.18em] text-[#ba0c2f]">Kariyer Pusulası sonucunuz</p><h2 className="mt-2 text-3xl font-black">Doğal avantajınızı işe dönüştürün.</h2></div>
              <button onClick={shareResult} className="inline-flex items-center gap-2 rounded-xl border border-black/10 px-4 py-2.5 text-sm font-semibold transition hover:bg-neutral-50">{copied ? <Copy size={17} /> : <Share2 size={17} />}{copied ? "Bağlantı kopyalandı" : "Sonucu paylaş"}</button>
            </div>
            <p className="mt-6 text-base leading-7 text-neutral-600">{primary.summary}</p>
            <div className="mt-7 grid gap-4 sm:grid-cols-3">
              {primary.strengths.map((strength) => <div key={strength} className="rounded-2xl p-4" style={{ backgroundColor: primary.soft }}><Check size={18} style={{ color: primary.color }} /><p className="mt-2 text-sm font-bold">{strength}</p></div>)}
            </div>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-2">
          ...
          <article className="rounded-[1.75rem] border border-black/10 bg-[#1e1b1c] p-7 text-white sm:p-8">
        ...
        <section className="mt-6 rounded-[2rem] border border-black/10 bg-white p-6 sm:p-9">
          {!submitted ? (
            <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-start">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#ba0c2f]">Sonraki adım</p>
                <h2 className="mt-2 text-3xl font-black">Bu profil gayrimenkulde nasıl çalışır?</h2>
              ...
              <form onSubmit={submitLead} className="grid gap-4 sm:grid-cols-2">
                <label className="text-sm font-semibold">Görüşmek istediğiniz ofis<select name="office" defaultValue="Kararsızım" className="mt-1.5 w-full rounded-xl border border-black/15 bg-white px-3 py-3 font-normal outline-none focus:border-[#ba0c2f]"><option>Kararsızım</option><option>KW Alesta</option><option>KW Viya</option><option>KW Orsa</option></select></label>
                <label className="text-sm font-semibold">Gayrimenkul deneyimi<select name="experience" defaultValue="Deneyimim yok" className="mt-1.5 w-full rounded-xl border border-black/15 bg-white px-3 py-3 font-normal outline-none focus:border-[#ba0c2f]"><option>Deneyimim yok</option><option>1 yıldan az</option><option>1-3 yıl</option><option>3 yıldan fazla</option></select></label>
                <label className="text-sm font-semibold">Uygun zaman<select name="preferred_time" defaultValue="En kısa sürede" className="mt-1.5 w-full rounded-xl border border-black/15 bg-white px-3 py-3 font-normal outline-none focus:border-[#ba0c2f]"><option>En kısa sürede</option><option>Hafta içi 09:00-12:00</option><option>Hafta içi 12:00-17:00</option><option>Hafta içi 17:00 sonrası</option></select></label>
                <input name="website" tabIndex={-1} autoComplete="off" className="absolute -left-[9999px] h-px w-px" aria-hidden="true" />
