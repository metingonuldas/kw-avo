...
export default function PropertyLeadThankYou({ allowRental = false }: { allowRental?: boolean }) {
  return (
    <main className="relative isolate flex min-h-[72vh] items-center overflow-hidden bg-neutral-950 px-4 py-16 text-white sm:px-6">
      ...
      <div className="mx-auto w-full max-w-3xl rounded-[32px] border border-white/15 bg-white/10 p-7 text-center shadow-2xl backdrop-blur sm:p-12">
        ...
        <p className="mx-auto mt-5 max-w-xl text-base leading-7 text-neutral-300">
          Bölge ekibimiz mülkünüz ve {allowRental ? "satış veya kiraya verme" : "satış"} hedefiniz hakkında kısa bir ön görüşme yapmak için belirttiğiniz zaman aralığında sizinle iletişime geçecek.
        </p>
        ...
        <div className="mt-9 grid gap-3 text-left sm:grid-cols-3">
          ...
          <div className="rounded-2xl border border-white/10 bg-black/20 p-4"><PhoneCall className="text-red-300" size={21} /><strong className="mt-3 block text-sm">Kısa ön görüşme</strong><span className="mt-1 block text-xs leading-5 text-neutral-400">Mülkünüzü ve {allowRental ? "hedefinizi" : "satış hedefinizi"} dinleriz</span></div>
